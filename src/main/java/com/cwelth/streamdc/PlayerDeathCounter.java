...
public class PlayerDeathCounter {
    ...
    public static void sendRankTable(PlayerEntity ep, List<PlayerDeathCounter> playerDeathCounters)
    {
        ep.sendMessage(new StringTextComponent(TextFormatting.WHITE + "Top 5 deaths on server:"));
        ...
        for (int i = 0; i < maxIteration; i++) {
            if(playerDeathCounters.get(i).getUUID().equals(ep.getUniqueID().toString()))
                ep.sendMessage(new StringTextComponent(TextFormatting.fromColorIndex(i+1) + "[->] "+playerDeathCounters.get(i).getNickname()+" - " + TextFormatting.BOLD + playerDeathCounters.get(i).getDeathCount()));
            else
                ep.sendMessage(new StringTextComponent(TextFormatting.fromColorIndex(i+1) + "[  ] " +playerDeathCounters.get(i).getNickname()+" - " + TextFormatting.BOLD + playerDeathCounters.get(i).getDeathCount()));
        }
    }
